...
plt.title("Diamond Princess ship's cumulative cases", fontsize = 30)
plt.xlabel('Date (February, 2020)', fontsize = 25)
plt.ylabel('Poplution', fontsize = 25)
...
plt.title("2003 Hong Kong SARS' cumulative cases", fontsize = 30)
plt.xlabel('Day (March ~ June, 2003)', fontsize = 25)
plt.ylabel('Poplution', fontsize = 25)
...
no_nan_Beta_SR = Beta_SR[~ np.isnan(Beta_SR)]

print("Beta (2003 香港 SARS):\n", no_nan_Beta_SR)
print("\n平均 Beta :", no_nan_Beta_SR.mean())
...
plt.title("The infection transmission rate (Diamond Princess ship)", fontsize = 30)
...
plt.title("The infection transmission rate (2003 Hong Kong SARS)", fontsize = 30)
...
plt.title("Logistic Fitting Curve (2003 Hong Kong SARS)", fontsize = 30)
plt.xlabel("Day (March ~ June, 2003)", fontsize = 25)
plt.ylabel("Population", fontsize = 25)
...
plt.title("R0 (Diamond Priness' ship)", fontsize = 30)
plt.xlabel('Date (February, 2020)', fontsize = 25)
plt.ylabel('Poplution', fontsize = 25)
date =('04', '05', '06', '07', '08', '09', '10', '12', '13', '15', '16', '17', '18', '19', '20', '26')
plt.xticks(np.arange(0, 16, 1), date)
...
Beta_SR = Beta_SR[~ np.isnan(Beta_SR)]
...
print("\n差分方程得出的平均Beta、20~40 天的 N = 1601、病程 7 天，則 R0 =", r0_L2_1SR)
...
print("\n差分方程得出的平均Beta、20~50 天的 N = 1618、病程 7 天，則 R0 =", r0_L3_1SR)
...
plt.title("R0 (2003 Hong Kong SARS)", fontsize = 30)
plt.xlabel('Day (March ~ June, 2003)', fontsize = 25)
plt.ylabel('Poplution', fontsize = 25)
plt.xticks(np.arange(1, 83, 1))
...
def plotSIR_D(N, beta, gamma, T):
    ...
    plt.title("Diamond Princess' ship\nSIR Model (Beta = %g, Viral Shedding : %s days)" % tuple(v), fontsize = 30)
...
def plotIR_D(N, beta, gamma, T, col, d):
    ...
    plt.title("Diamond Princess' ship\nI+R Plot, N: 3711, Beta: %g" % beta, fontsize = 30)
...
def plotIR_SR(N, beta, gamma, T, col, d):
    ...
    plt.plot(T_range, RES[:, 1]+RES[:, 2], '-', color=col, label='%s' % d)
    plt.title("2003 Hong Kong SARS\nI+R Plot, N: 1618, Beta: %g" % beta, fontsize = 30)
    plt.xticks(np.arange(0, T+1, 5))
    plt.yticks(np.arange(0, 1701, 50))
    plt.xlabel('Day', fontsize = 25)
    plt.ylabel('Population', fontsize = 25)
